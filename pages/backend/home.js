...
function BackendHome(props) {
  ...
  const ConfirmDelete = async function () {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .post("http://localhost:8080/apiV1/item/delete", {
            name: name,
          })
          .then((res) => {
  ...
  if (menu == "overview") {
    return (
      <Container>
        ...
        <h1 className="ml-5 text-base text-white/60">
          ผู้ใช้งานในเว็ปทั้งหมด {countUser} บัญชี
        </h1>
        <Button
          variant="outline-success"
          className="ml-5 text-white bg-gradient-to-t from-main-600 to-pink-400 px-2 rounded-2xl"
        ...
        <div className="border-t border-white w-full my-3 opacity-40"></div>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 gap-x-4 text-green-500 justify-center text-center pt-7 px-4">
          {props.store.datauser.data.data.map((userdata, index) => (
            <div
              key={index}
              className="rounded-2xl shadow-xl border-8 bg-white border-green-500 p-6 transition ease-in-out delay-50 hover:-translate-y-7 hover:scale-11 duration-300 cursor-pointer"
              onClick={() => showUserData(userdata.username)}
            >
        ...
        <h1 className="ml-5 text-base text-white/60">
          สินค้าในเว็ปทั้งหมด {countItem} ชิ้น
        </h1>
        <div className="border-t border-white w-full my-3 opacity-40"></div>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 gap-x-4 text-green-500 justify-center text-center pt-7 px-4">
          {props.store.dataitem.data.data.map((itemdata, index) => (
            <div
              key={index}
              className="rounded-2xl shadow-xl border-8 bg-white border-green-500 p-6 transition ease-in-out delay-50 hover:-translate-y-7 hover:scale-11 duration-300 cursor-pointer"
              onClick={() => showItemData(itemdata.name)}
            >
...
//     "https://golang-authapi.onrender.com/apiV1/items"
//   );
//   const dataitem = resitem.data;
//   const resuser = await axios.get(
//     "https://golang-authapi.onrender.com/apiV1/users"
//   );
